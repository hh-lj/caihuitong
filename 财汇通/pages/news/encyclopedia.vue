<template>
  <view class="case-body">
    ...
    <view class="case-list">
      <view class="content">
        <view class="case-nav">
          <navigator v-for="(item,index) in list_column_next" :key="index" :url="'http://www.cdcaihuitong.com' + item.url"
            :class="{'active':current==index}" @click="addClass(index)">
            <view class="drawer-item">
              {{item.name}}
            </view>
          </navigator>
       ...
       <view class="case-item" v-for="(item,index) in list_document.data" :key="index">
        ...
        <uni-pagination @change="handlePage($event)" show-icon="true" :total="postCount" :current="pageNum" :pageSize="pageSize"></uni-pagination>
      </view>
    </view>
      ...
      uni.request({
        ...
        success: (res) => {
          console.log(res),
            ...
            this.list_column_next = res.data.data.list_column_next,
